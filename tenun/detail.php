...
include_once './../src/c.php';
    ...
    $sql_query = "SELECT * FROM informasi where slug='$slug'";
    ...
    <nav class="w-full mx-auto px-4 sm:px-6 lg:px-8 sm:flex sm:items-center sm:justify-between" aria-label="nav">
      ...
      <div id="navbar-image-1" class="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow sm:block">
        <div class="flex flex-col gap-5 mt-5 sm:flex-row sm:items-center sm:justify-end sm:mt-0 sm:ps-5">
          ...
          <a class="font-medium text-blue-500" href="#" aria-current="page">Kain Tenun</a>
  ...
  <section>
    <?php
        if ($res = $result->fetch_object()) {
            ...
            $cover = $res->cover;
            $created_at = date_create($res->created_at);
            $created_at = date_format($created_at,"d F, Y");
    ?>
    <!-- Blog Article -->
    <div class="container px-4 pt-6 lg:pt-10 pb-12 sm:px-6 lg:px-8 mx-auto">
    <div class="w-full">
        <a class="inline-flex items-center gap-x-1.5 mb-10 text-sm text-gray-600 decoration-2 hover:underline" href="./index.php">
        ...
        <div class="space-y-3">
            <h2 class="text-3xl font-bold lg:text-5xl"><?= $judul ?></h2>
            <div class="flex w-full sm:items-center gap-x-5 sm:gap-x-3">
            <div class="flex-shrink-0">
            <img class="size-12 rounded-full" src="./../dist/images/informasi/<?= $cover ?>" alt="Image Description">
            </div>
    ...
    <div class="container py-20 text-center">
        <h2 class="text-2xl font-bold md:text-3xl">Data tidak ditemukan</h2>
    </div>
    <?php
        }
    ?>
